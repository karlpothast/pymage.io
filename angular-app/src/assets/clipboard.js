...
//region "call jquery function"
   $(function(){
      ...
      $('.demo').on('pasteImage', function(ev, data){
        clear();
        pleaseWait(true);
        var blobURL = URL.createObjectURL(data.blob).toString();
        blobURL = blobURL.replace('blob:','')
        var base64 = data.dataURL;
        base64 = base64.replace('data:image/png;base64,','');
        postBase64(base64, blobURL);
        //console.log('data :' + data);
        //console.log('base64 var ' + base64);
      }).on('pasteImageError', function(ev, data){
          errmsg = "Your browser settings do not allow access to your clipboard.";
          displayErrorMessage(msg);
          return;
      }).on('pasteText', function(ev, data){
        errmsg = "No image found. Screenshot an image with text and try again.";
...
$(function(){
  $('#pasteButton').on('click', function(ev, data){
    getClipboardImage();
  });
});
async function getClipboardImage() {
  ...
  try {
    ...
    if (permission.state === "denied") {
        msg = "Your browser settings do not allow access to your clipboard.";
        displayErrorMessage(msg);
        return;
    }
    ...
    for (const item of clipboardContents) {
      if (!item.types.includes("image/png")) {
        msg = "No image data found. Screenshot an image with text and try again.";
        displayErrorMessage(msg);
        return;
      }
      const data = await item.getType("image/png");
      var blobURL = URL.createObjectURL(data).toString();
      blobURL = blobURL.replace('blob:','');

      let readerBase64 = null;
      let reader = null;
      reader = new FileReader();
      reader.readAsDataURL(data); 
      reader.onloadend = function() {
        if (!readerBase64) {
          readerBase64 = reader.result;  
          readerBase64 = readerBase64.replace('data:image/png;base64,','');
          postBase64(readerBase64, blobURL);
        }   
      }
    }
  } 
...
function pleaseWait(visible){
  const divPleaseWait = document.getElementById("divPleaseWait");
  if (visible) {
...
function ocrComplete(visible) {
  const completedDiv = document.getElementById("completedDiv");
  if (completedDiv) {
    if (visible) {
      completedDiv.style.visibility= "visible";
    } else {
      completedDiv.style.visibility= "hidden";    
    }
  }

  const pMsg = document.getElementById("pMsg");
  pMsg.style.visibility="visible";
  pMsg.style.color="#306998";
  pMsg.innerText = "Text extracted successfully!";
 
  const ocrtext = document.getElementById("ocrtext");
  ocrtext.style.color="black";
...
function postBase64(base64String, blobURL) {
  var bodyPostString;
  bodyPostString = `{
    \"base64String\": \"${base64String}\",
    \"blobURL\": \"${blobURL}\"
  }`;
  var postURL = 'https://localhost:5001/api/ocr/PostBase64';
  ...
  fetch(postURL, {
    ...
    .then(function(data){ 
      var jsonReturn = data.extracted_text;
      ocrtext.value = jsonReturn;
      pleaseWait(false);
      ocrComplete(true);
